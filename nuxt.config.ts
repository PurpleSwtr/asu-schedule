...
function buildGroupsManifest() {
  ...
  const files = readdirSync(publicDir).filter(
    (f) => f.endsWith(".json") && f !== "groups-manifest.json",
  )
  ...
  writeFileSync(
    join(publicDir, "groups-manifest.json"),
    JSON.stringify({ groups }, null, 2),
  )
  console.log(
    `[groups-manifest] generated ${groups.length} group(s) into public/`,
  )
}
    ...
    head: {
      ...
      meta: [
        { name: "viewport", content: "width=device-width, initial-scale=1" },
        {
          name: "description",
          content:
            "Расписание занятий МАДИ. Числитель и знаменатель по неделям.",
        },
      ],
    },
  },
